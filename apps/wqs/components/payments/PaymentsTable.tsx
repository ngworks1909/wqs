...
function StatusBadge({ status }: { status: PaymentStatus }) {
  const statusColors: Record<PaymentStatus, { bg: string; text: string }> = {
    ...
    [PaymentStatus.Success]: { bg: 'bg-green-100', text: 'text-green-800' },
    [PaymentStatus.Failed]: { bg: 'bg-red-100', text: 'text-red-800' },
  };

  const colors = statusColors[status] || statusColors[PaymentStatus.Pending];

  return (
    <span className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${colors.bg} ${colors.text}`}>
      {status}
    </span>
  );
}
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="cursor-help">
                          {truncateText(payment.request.sampleLocation, 15)}
                        </span>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{payment.request.sampleLocation}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </td>
                <td className="hidden px-4 py-3 lg:table-cell">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="cursor-help">
                          {truncateText(payment.request.waterType.name, 15)}
                        </span>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{payment.request.waterType.name}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </td>
